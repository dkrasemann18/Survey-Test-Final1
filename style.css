...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  function normalizeTask(task) {
    ...
    if (t.startsWith('brainstorm')) return 'Brainstorm';
    if (t.startsWith('drafting')) return 'Drafting & Writing';
    return task;
  }
  ...
  function processForm() {
    ...
    const tools = formData.getAll('tools') || []; // <- capture selected tools
    ...
    if (favorites && favorites.trim() !== '') {
      ...
      const body = encodeURIComponent(
        ...
        'Selected Tasks: ' + (tasks && tasks.length ? tasks.join(', ') : 'None selected') + '\n' +
        'Favorite Prompt: ' + favorites.trim() + '\n' +
        'Tools Used: ' + (tools.length ? tools.join(', ') : 'None selected')
      );

      const mailtoUrl = 'mailto:dkrasemann@deloitte.com?subject=' + subject + '&body=' + body;

      // Try to open the mail draft; still move to results even if blocked by the client.
      try {
    ...
    // Filter prompts based on answers
    ...
    // Group prompts by task category
    const grouped = {};
